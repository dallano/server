...
entity.onTrigger = function(player, npc)
    ...
    if not player:hasItem(18246 + playerRace - raceOffset) then
        npcUtil.popFromQM(player, npc, ID.mob.AROMA_LEECH, { claim = true, hide = 600, look = true, radius = 1 })  -- ??? despawns and respawns 10 minutes after NM dies
        ...
        GetMobByID(ID.mob.AROMA_LEECH):addListener("ITEM_DROPS", "ITEM_DROPS_RSE", function(mob, loot)
            loot:addItem(item, xi.drop_rate.VERY_NCOMMON)
        end)
